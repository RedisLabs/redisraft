...
RRStatus checkLeader(RedisRaftCtx *rr, RedisModuleCtx *ctx, Node **ret_leader)
{
    ...
    if (raft_node_get_id(leader) != raft_get_nodeid(rr->raft)) {
        ...
        if (leader_node) {
            ...
            replyRedirect(ctx, 0, &leader_node->addr);
        } else {
            RedisModule_ReplyWithError(ctx, err_clusterdown);
        }
        return RR_ERROR;
...
void joinLinkIdleCallback(Connection *conn)
{
    ...
    LOG_VERBOSE("%s cluster, connecting to %s:%u", state->type,
...
exit_fail:
    ...
    snprintf(err, sizeof(err), "ERR failed to %s cluster, please check logs", state->type);
    RedisModule_ReplyWithError(state->req->ctx, err);
